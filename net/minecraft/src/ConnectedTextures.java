...
public class ConnectedTextures
{
    ...
    private static boolean isNeighbour(ConnectedProperties cp, IBlockAccess iblockaccess, Block block, int x, int y, int z, int side, IIcon icon, int metadata)
    {
        ...
        if (cp.connect == 2)
        {
            ...
            else
            {
                ...
                if (side >= 0)
                {
                    neighbourIcon = neighbourBlock.getIcon(iblockaccess, x, y, z, side);
                }
                else
                {
                    neighbourIcon = neighbourBlock.getIcon(iblockaccess, x, y, z, 1);
                }
    ...
    private static IIcon getConnectedTextureHorizontal(ConnectedProperties cp, IBlockAccess blockAccess, Block block, int x, int y, int z, int side, IIcon icon, int metadata)
    {
        if (side != 0 && side != 1)
        {
            boolean left = false;
    ...
    private static IIcon getConnectedTextureVertical(ConnectedProperties cp, IBlockAccess blockAccess, Block block, int x, int y, int z, int side, IIcon icon, int metadata)
    {
        if (side != 0 && side != 1)
        {
    ...
    private static IIcon getConnectedTextureTop(ConnectedProperties cp, IBlockAccess blockAccess, Block block, int x, int y, int z, int side, IIcon icon, int metadata)
    {
